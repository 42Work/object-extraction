    ...
    def freq1(self, data, frequent_num):
        ...
        for i in range(len(data)):
            for j in range(len(data[i])):
                appear.append(data[i][j])
        # print(appear_ele)
        # print(appear_ele2)
        appear_alone=set(appear)
        for item in appear_alone:
            itmes = appear.count(item)
...
with open('./data/data_gsp.pkl','rb') as f:
    data=pickle.load(f)
starttime = datetime.datetime.now()
s = GSP()
freq1 = s.freq1(data, 3)
s.freq_more(data, freq1)
endtime = datetime.datetime.now()
print(endtime - starttime)

with open('./data/final.pkl','wb') as f1:
    pickle.dump(s,f1)